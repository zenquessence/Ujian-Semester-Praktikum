...
if (isset($_POST['simpan'])) {
    ...
    // Cek Duplikasi Kode Kamera
    $cek = mysqli_query($koneksi, "SELECT * FROM kamera WHERE kode_kamera='$kode'");
    if (mysqli_num_rows($cek) > 0) {
        echo "<script>alert('Kode Kamera sudah terdaftar!'); window.history.back();</script>";
        return false;
    }
